...
/// 
void generateTrainedOutput(NeuralNet& net, const vector<vector<double> >& inVecs, 
										   const vector<vector<double> >& targetVecs, double scale, const string& fname)
{
	...
	if(outFile.good())
	{
		...
		outFile << "Predictor (X),Actual Value,Response (Y)" << endl;
		...
		for(int i = 0; i < size; i++)
		{
			...
			outFile << dX[0] / scale << "," << targetVecs[i][0] / scale << ',' << dY[0] / scale << endl;
		}
					  ...
					  const string& outputFile)
{
	...
	if(createTrainingSet(dataFile, xCol, yCol, scaleFac, inVecs, targetVecs) == 0)
	{
		...
		generateTrainedOutput(net, inVecs, targetVecs, scaleFac, outputFile);
...
void validateModelFit(double xStart, double xEnd, double xStep, double scale, const string& netFname, const string& outFname)
{
	...
	if(outFile.good())
	{
		...
		// output the column titles
		outFile << "Predictor (X),Response (Y)" << endl;
